...
public class UsuarioController {
	...
	public ResponseEntity<Map<String, Object>> obtenerUsuarioPorId(@PathVariable Integer idUsuario) {
		try {
			...
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(Map.of("mensaje",
					"Error al obtener el usuario", "httpStatus", HttpStatus.INTERNAL_SERVER_ERROR.value()));
		}
	}
	...
	public ResponseEntity<Map<String, Object>> login(@RequestBody LoginDTO user) {
		try {
			...
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(Map.of("mensaje",
					"Error al intentar iniciar sesión", "httpStatus", HttpStatus.INTERNAL_SERVER_ERROR.value()));
		}
	}
	...
	public ResponseEntity<Map<String, Object>> cambiarEstado(@PathVariable("id") Integer idUsuario) {
		try {
			...
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(Map.of("mensaje",
					"Error al cambiar el estado del usuario", "httpStatus", HttpStatus.INTERNAL_SERVER_ERROR.value()));
		}
	}
	...
	public ResponseEntity<Map<String, Object>> obtenerMiPerfil(Authentication authentication) {
		try {
			...
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(Map.of("mensaje",
					"Error al obtener el perfil del usuario", "httpStatus", HttpStatus.INTERNAL_SERVER_ERROR.value()));
		}
	}
	...
	public ResponseEntity<Map<String, Object>> obtenerPerfil(HttpServletRequest request){
		...
		try {
			return usuarioService.obtenerDatosPorToken(request);
		} catch (Exception e) {
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(Map.of("mensaje",
					"Error al obtener el perfil del usuario", "httpStatus", HttpStatus.INTERNAL_SERVER_ERROR.value()));
		}		
